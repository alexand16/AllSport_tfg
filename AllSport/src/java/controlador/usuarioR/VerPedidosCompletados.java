 ...
 * @author alanr
 */
public class VerPedidosCompletados extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String vista = "/usuarioR/verPedidosCompletados.jsp";
        ...
        List<Pedidos> pedidos = pejc.findPedidosByIDCliente(cliente.getId());
        List<Pedidos> pedidosCompletados = new ArrayList<>();
        for (int i = 0; i < pedidos.size(); i++) {
            if (pedidos.get(i).getEstadoPedido() != null) {
                if (pedidos.get(i).getEstadoPedido().equals("entregado")) {
                    pedidosCompletados.add(pedidos.get(i));
                }
            }
        }
